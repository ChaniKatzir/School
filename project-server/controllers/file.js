...
const pdfFilePath = './files/filledReportFile';
...
async function fillPdfForm(inputPdfPath, outputPdfPath, fieldData) {
    ...
    // await fs.writeFile('C:\\Users\\65282\\Downloads', finalPdfBytes);

    // downloadPDF(outputPdfPath, 'C://downloads')
    ...
    // console.log("pppppppppppppppppppppppppp",pdfDoc.getPageCount());
    ...
    // // Save the modified PDF with the new page
    // const finalPdfBytes = await pdfDoc.save();
    // await fs.writeFile(outputPdfPath, finalPdfBytes);
}
...
exports.print = async (req, res) => {
    let student = await students.findOne({ where: { id: 1 } });
    let year = new Date().getFullYear();
    let name = student.name;
    let classes = ["א", "ב", "ג", "ד", "ה", "ו", "ז", "ח"];
    let marks = await subject_marks.findAll();
    marks = marks.map(mark => mark.dataValues.name);
    let allAssessments = await assessments.findAll();
    allAssessments = allAssessments.map(assessment => assessment.dataValues.name);
    let MMarks = await manner_marks.findAll();
    MMarks = MMarks.map(mark => mark.dataValues.name);

    let allManners = await manners.findAll();
    allManners = allManners.map(manner => manner.dataValues.name);
    ...
    let period = req.params.periodB + " " + req.params.periodA;
    // const fieldData = {
    ...
    // };
    // console.log("0000000000000", MMarks);
...
//     const pdfData = fs.readFileSync('C:\\Users\\65282\\Desktop\\reportFile.pdf');

//     // Insert the PDF template into the database
//     const insertQuery = 'INSERT INTO pdfForm (name, file) VALUES (?, ?)';
//     const templateName = 'report'; // Provide a meaningful name
//     connection.query(insertQuery, [templateName, pdfData], (error, results) => {
//         if (error) {
//             console.error('Error inserting PDF template:', error);
//             console.log("results", results);
//         } else {
//             console.log('PDF template inserted successfully!');
//             console.log("results", results);
//         }
//         connection.end();
//     })
...
//     const selectQuery = 'SELECT file FROM pdfForm WHERE name = ?';
//     const templateNameToRetrieve = 'report'; // Provide the template name you want to retrieve
//     connection.query(selectQuery, [templateNameToRetrieve], (error, results) => {

//         if (error) {
//             console.error('Error retrieving PDF template:', error);
//             res.send({
//                 message: "Content can not be empty!" || error
//             });
//         } else if (results.length > 0) {
//             const pdfData = results[0]?.template_data;
//             // Generate a filled PDF using pdfkit
//             const filledPdfDoc = new PDFDocument();

//             // Modify form fields or add content based on your requirements
//             filledPdfDoc.text('Filled PDF Content');

//             // // Save the filled PDF to a file
//             // const filledPdfPath = 'path/to/filled.pdf';
//             // const filledPdfStream = fs.createWriteStream(filledPdfPath);
//             // filledPdfDoc.pipe(filledPdfStream);
//             // filledPdfDoc.end();

//             // filledPdfStream.on('finish', () => {
//             //     console.log('Filled PDF saved successfully:', filledPdfPath);
//             //     connection.end();
//             // });
//             ///print///////////////////////////////////////////////////////////////////
//             res.send(pdfData);
//             // Use the retrieved binary data as needed (e.g., generate a filled PDF)
//         } else {
